...
internal static class EvmObjectFormatSS
{
    ...
    internal class Eof1 : IEofVersionHandler
    {
        ...
        internal const byte KIND_TYPE_OFFSET = VERSION_OFFSET + EvmObjectFormatSS.ONE_BYTE_LENGTH; // version length
        internal const byte TYPE_SIZE_OFFSET = KIND_TYPE_OFFSET + EvmObjectFormatSS.ONE_BYTE_LENGTH; // kind type length
        internal const byte KIND_CODE_OFFSET = TYPE_SIZE_OFFSET + EvmObjectFormatSS.TWO_BYTE_LENGTH; // type size length
        internal const byte NUM_CODE_SECTIONS_OFFSET = KIND_CODE_OFFSET + EvmObjectFormatSS.ONE_BYTE_LENGTH; // kind code length
        internal const byte CODESIZE_OFFSET = NUM_CODE_SECTIONS_OFFSET + EvmObjectFormatSS.TWO_BYTE_LENGTH; // num code sections length
        internal const byte KIND_DATA_OFFSET = CODESIZE_OFFSET + DYNAMIC_OFFSET; // all code size length
        internal const byte DATA_SIZE_OFFSET = KIND_DATA_OFFSET + EvmObjectFormatSS.ONE_BYTE_LENGTH + DYNAMIC_OFFSET; // kind data length + all code size length
        internal const byte TERMINATOR_OFFSET = DATA_SIZE_OFFSET + EvmObjectFormatSS.TWO_BYTE_LENGTH + DYNAMIC_OFFSET; // data size length + all code size length
        internal const byte HEADER_END_OFFSET = TERMINATOR_OFFSET + EvmObjectFormatSS.ONE_BYTE_LENGTH + DYNAMIC_OFFSET; // terminator length + all code size length
        ...
        internal const ushort MINIMUM_SIZE = HEADER_END_OFFSET
                                            + EvmObjectFormatSS.TWO_BYTE_LENGTH // one code size
        ...
        public static int CalculateHeaderSize(int codeSections) =>
            HEADER_END_OFFSET + codeSections * EvmObjectFormatSS.TWO_BYTE_LENGTH;
        ...
        public bool TryParseEofHeader(ReadOnlySpan<byte> container, out EofHeader? header)
        {
            ...
            int codeSizeLenght = numberOfCodeSections * EvmObjectFormatSS.TWO_BYTE_LENGTH;
            ...
            {
                int currentCodeSizeOffset = CODESIZE_OFFSET + pos * EvmObjectFormatSS.TWO_BYTE_LENGTH; // offset of pos'th code size
