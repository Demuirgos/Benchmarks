...
internal static class EvmObjectFormatPSDoubleReachableCodeOpt
{
    ...
    static EvmObjectFormatPSDoubleReachableCodeOpt()
    {
    ...
    internal class Eof1 : IEofVersionHandler
    {
        ...
        internal const byte KIND_TYPE_OFFSET = VERSION_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.ONE_BYTE_LENGTH; // version length
        internal const byte TYPE_SIZE_OFFSET = KIND_TYPE_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.ONE_BYTE_LENGTH; // kind type length
        internal const byte KIND_CODE_OFFSET = TYPE_SIZE_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH; // type size length
        internal const byte NUM_CODE_SECTIONS_OFFSET = KIND_CODE_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.ONE_BYTE_LENGTH; // kind code length
        internal const byte CODESIZE_OFFSET = NUM_CODE_SECTIONS_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH; // num code sections length
        internal const byte KIND_DATA_OFFSET = CODESIZE_OFFSET + DYNAMIC_OFFSET; // all code size length
        internal const byte DATA_SIZE_OFFSET = KIND_DATA_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.ONE_BYTE_LENGTH + DYNAMIC_OFFSET; // kind data length + all code size length
        internal const byte TERMINATOR_OFFSET = DATA_SIZE_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH + DYNAMIC_OFFSET; // data size length + all code size length
        internal const byte HEADER_END_OFFSET = TERMINATOR_OFFSET + EvmObjectFormatPSDoubleReachableCodeOpt.ONE_BYTE_LENGTH + DYNAMIC_OFFSET; // terminator length + all code size length
        ...
        internal const ushort MINIMUM_SIZE = HEADER_END_OFFSET
                                            + EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH // one code size
        ...
        public static int CalculateHeaderSize(int codeSections) =>
            HEADER_END_OFFSET + codeSections * EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH;
        ...
        public bool TryParseEofHeader(ReadOnlySpan<byte> container, out EofHeader? header)
        {
            ...
            int codeSizeLenght = numberOfCodeSections * EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH;
            ...
            {
                int currentCodeSizeOffset = CODESIZE_OFFSET + pos * EvmObjectFormatPSDoubleReachableCodeOpt.TWO_BYTE_LENGTH; // offset of pos'th code size
        ...
        public bool ValidateStackState(int sectionId, in ReadOnlySpan<byte> code, in ReadOnlySpan<byte> typesection, ushort worksetCount)
        {
            ...
            {
                return (workset[worksetPointer++], workset[worksetPointer++]);
            }
            static void PushWorklet(ushort dest, ushort height, ushort[] workset, ref ushort worksetTop)
            {
                workset[worksetTop++] = dest;
                workset[worksetTop++] = height;
            }

            try {
                PushWorklet(0, (ushort)peakStackHeight, workset, ref worksetTop);
                while (worksetPointer < worksetTop)
                {
                    (int pos, int stackHeight) = PopWorklet(workset, ref worksetPointer);
                    bool stop = false;
                    ...
                    while (!stop)
                    {
                        ...
                        if (recordedStackHeight[pos] != 0)
                        {
                            if (stackHeight != recordedStackHeight[pos] - 1)
                            {
                                return false;
                            }
                            break;
                        ...
                        {
                            recordedStackHeight[pos] = (short)(stackHeight + 1);
                        }
                            ...
                            case Instruction.RJUMP:
                                {
                                    ...
                                    PushWorklet((ushort)jumpDestination, (ushort)stackHeight, workset, ref worksetTop);
                                    stop = true;
                                    linesCount += immediates;
                                    break;
                                }
